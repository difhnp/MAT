...
class LVISLoader(DatasetLoader):
    def __init__(self, name: str, split: str):
        super(LVISLoader, self).__init__()
        ...
        if split == 'train':
            self.root = '/data1/Datasets/LVIS_v1/'
        elif split == 'val':
            self.root = '/data1/Datasets/LVIS_v1/'
        # elif split == 'test':
        #     self.root = '/data1/Datasets/LVIS_v1/'
        ...
        if split == 'train':
            ...
            with open(os.path.join(self.root, 'lvis_v1_train.json'), 'r') as fin:
                self.meta = json.load(fin)
            self.video_list = self.meta['annotations']

        elif split == 'val':

            with open(os.path.join(self.root, 'lvis_v1_val.json'), 'r') as fin:
                self.meta = json.load(fin)
            self.video_list = self.meta['annotations']
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    # #################################
    # [lvis_v1_train] -- 1270141 videos, 1270141 frames, done !
    # [lvis_v1_val] -- 244707 videos, 244707 frames, done !
    # #################################
    parser = argparse.ArgumentParser(description='parse LVIS_v1 for lmdb')
    ...
    dataset_name = f'lvis_v1_{args.split}'
    ...
    data_set = LVISLoader(name=dataset_name, split=args.split)
