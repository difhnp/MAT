...
class LaSOTLoader(DatasetLoader):
    def __init__(self, name: str, split: str):
        ...
        self.root = '/data1/Datasets/LaSOT/dataset/'
        self.anno_dir = os.path.join(self.root, 'annos/absent')
        self.data_dir = os.path.join(self.root, 'images')

        if split == 'train':
            with open('/data1/Datasets/LaSOT/dataset/training_set.txt', 'r') as f:
                video_list = f.readlines()
            self.video_list = sorted([x.split('\n')[0] for x in video_list])
        elif split == 'test':
            with open('/data1/Datasets/LaSOT/dataset/testing_set.txt', 'r') as f:
                video_list = f.readlines()
            self.video_list = sorted([x.split('\n')[0] for x in video_list])
        else:
    ...
    def get_video_info(self, video_name: str):
        ...
        img_list = sorted(os.listdir(os.path.join(video_dir, 'img')))
        img_list = np.array([os.path.join(video_name, 'img', img_file) for img_file in img_list])
...
if __name__ == '__main__':
    ...
    dataset_name = f'lasot_{args.split}'
